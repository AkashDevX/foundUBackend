...
@section('subheading')
    {{ $company->name }} — plan and edit shifts by employee and day (Monday–Sunday, {{ \App\Support\DisplayTimezone::label() }}).
@endsection
...
@section('content')
    ...
    <section class="mb-6 overflow-hidden rounded-2xl border border-brand-border bg-white shadow-sm ring-1 ring-black/[0.02]">
        <div class="flex flex-col gap-4 border-b border-brand-border bg-gradient-to-br from-brand-surface via-white to-white px-4 py-4 lg:flex-row lg:items-center lg:justify-between sm:px-6">
        ...
        <div class="grid gap-px bg-brand-border/70 sm:grid-cols-2 lg:grid-cols-4">
    ...
    @if ($scheduleRows === [])
        <div class="rounded-2xl border border-dashed border-brand-border bg-brand-surface/50 px-6 py-12 text-center">
            ...
            <p class="mt-2 text-xs text-brand-text-secondary">Try clearing filters or assigning active employees under Work assignments.</p>
        </div>
    @else
                                        ...
                                        <div class="flex min-h-[108px] flex-col gap-1.5">
                                            @if ($isDayOff && $blocks !== [])
                                                ...
                                                >
                                                    ...
                                                    <p class="mt-1 text-xs {{ $palette['text'] }} opacity-80">All day</p>
                                                    @if (($block['subtitle'] ?? '') !== '' && ($block['subtitle'] ?? '') !== 'Unavailable')
                                                        <p class="mt-2 text-xs font-medium {{ $palette['text'] }}">{{ $block['subtitle'] }}</p>
                                                    @endif
                                                    <p class="mt-2 text-[10px] font-semibold uppercase tracking-wide text-brand-text-secondary opacity-70">Click to edit or remove</p>
                                                </button>
                                            @else
                                            @foreach ($blocks as $block)
                                                ...
                                                >
                                                    ...
                                                    @endif
                                                    @if ($isSuggestion)
                                                        <p class="mt-2 text-[10px] font-semibold uppercase tracking-wide text-brand-text-secondary">Click to save</p>
                                                    @else
        ...
        <p class="mt-4 text-xs text-brand-text-secondary">
            Use <strong>+ Add shift</strong> or <strong>+ Day off</strong> in any cell. On a day off, use <strong>+ Add shift</strong> to schedule work instead. Click an existing card to edit or delete it.
