...
@section('subheading')
    ...
    {{ $company->name }} — mobile clock in/out history and HR timesheet approval (GPS geofence verified).
...
@section('content')
    ...
    <p class="mb-6 rounded-xl border border-brand-border bg-white px-4 py-3 text-sm text-brand-text-secondary shadow-sm">
        ...
        Review weekly timesheets and clock in/out punches on one page. Select an employee below to focus their records.
    ...
    <section class="overflow-hidden rounded-2xl border border-brand-border bg-white shadow-sm ring-1 ring-black/[0.02]">
        ...
        <header class="border-b border-brand-border bg-gradient-to-br from-brand-surface via-white to-white px-5 py-4 sm:px-6">
            ...
            <p class="mt-1 text-xs text-brand-text-secondary">Click a row to open their timesheet and punch history.</p>
    ...
    <section class="mt-8 overflow-hidden rounded-2xl border border-brand-border bg-white shadow-sm ring-1 ring-black/[0.02]">
        ...
        <header class="border-b border-brand-border bg-gradient-to-br from-brand-surface via-white to-white px-5 py-4 sm:px-6">
            ...
            <h2 class="text-base font-bold text-brand-text">Timesheet approval</h2>

            <p class="mt-1 text-xs text-brand-text-secondary">Last 12 weeks · pay weeks run Monday to Sunday</p>

        </header>

        <div class="p-5 sm:p-6">

            @include('admin.partials.employee-timesheet-approvals', [
    ...
    @if ($selectedEmployee)
        ...
        <section id="punch-records" class="mt-8 overflow-hidden rounded-2xl border border-brand-border bg-white shadow-sm ring-1 ring-black/[0.02] scroll-mt-6">
            ...
            <header class="flex flex-wrap items-center justify-between gap-3 border-b border-brand-border bg-gradient-to-br from-brand-surface via-white to-white px-5 py-4 sm:px-6">
                ...
                <div>
                    ...
                    <p class="mt-1 text-xs text-brand-text-secondary">Clock in/out punches · last 12 weeks</p>

                </div>

                <a href="{{ route('admin.employees.time-clock', array_filter(['timesheet_status' => ($timesheetStatusFilter ?? 'pending') !== 'pending' ? $timesheetStatusFilter : null])) }}" class="inline-flex items-center rounded-lg border border-brand-border px-3 py-2 text-xs font-semibold text-brand-text-secondary hover:bg-brand-surface">
    ...
    @elseif ($employees->isNotEmpty())
        ...
        <div class="mt-8 rounded-2xl border border-dashed border-brand-border bg-brand-surface/40 px-6 py-10 text-center">
            ...
            <p class="text-sm font-semibold text-brand-text">Select an employee for punch details</p>

            <p class="mt-2 text-xs text-brand-text-secondary">Choose <strong>Open</strong> on any row above to see their clock in/out history below the timesheet queue.</p>
